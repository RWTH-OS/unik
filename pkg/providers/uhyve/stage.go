...
func (p *UhyveProvider) Stage(params types.StageImageParams) (_ *types.Image, err error) {
	...
	if _, err := os.Stat(kernelPath); os.IsNotExist(err) {
		logrus.Debugf("program.bin does not exist, check for HermitCore's loader")
		HermitLoaderPath := filepath.Join(filepath.Dir(params.RawImage.LocalImagePath), "ldhermit.elf")
		if _, err := os.Stat(HermitLoaderPath); os.IsNotExist(err) {
			logrus.Debugf("ldhermit.elf does not exist, assuming classic bootloader")
		} else {
			logrus.Infof("HermitCore's loader is used")
			if err := unikos.CopyFile(HermitLoaderPath, getHermitLoaderPath(params.Name)); err != nil {
				return nil, errors.New("copying hermit loader to image dir", err)
			}
		}
